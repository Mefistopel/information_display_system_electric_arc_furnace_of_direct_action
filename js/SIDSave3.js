    ...
    window.endPoints = [ //слитки расплавлены
     ...
     window.len = true; window.LenFifth = true;
           function rewrite(){ //перезапись объектов, чтобы появляющиеся после объекты не накладывались сверху на печь
...
function patternRunThird() { //кнопка ОТКЛЮЧИТЬ СВЕЧИ
  FuncCondition ('Простой', '#8B008B');StopCooling = true
    ...
    for (var i=0; i<3; i++){ //здесь отключаются свечи
     ...
     for (s = 0; s<5; s++){ //удаляем плавленные объекты
           canvas.remove(polygon[s]);
     }
     for (var i=0; i<metal_one_kind.length; i++){ //удаляем все объекты
...
function betaTest() {
  ...
  setTimeout(function(){
    ...
    window.StopCooling = true;
    delete stop, even, proviso, timer,StopWeight, stopTimer1; //удаляем переменные
    delete t, stopTimer1, StopWeight, stopTimerWei, stopTimerTem, StopCooling; //удаление переменных, т.к. они создаются при нажатии на кнопку
...
function animatePoint3(r, prop, endPoints3) { //сама анимация объекта: "выливание" и "заливание"
...
Также номер телефона: +7-923-031-64-46.
